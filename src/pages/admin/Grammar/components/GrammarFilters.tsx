...
interface GrammarFiltersProps {
  ...
  setFilters: React.Dispatch<React.SetStateAction<GrammarQueryParams>>;
  totalRecords: number;
  grammarCount: number;
  clearFilters: () => void;
  handleSearch: () => void;
        ...
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-center">
            <div className="text-sm text-gray-500">
              {t("common.showing")} {grammarCount} {t("common.of")} {totalRecords}
            </div>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col space-y-1.5">
                  <Label htmlFor="keyword">{t("adminGrammar.keyword")}</Label>
                  <div className="flex items-center gap-2">
                    <Input
                      ...
                      placeholder={t("adminGrammar.searchKeywordPlaceholder")}
                    />
                  </div>
                </div>
                
                <div className="flex flex-col space-y-1.5">
                  <Label htmlFor="level">{t("adminGrammar.level")}</Label>
                  <Select
                    value={filterInputs.level}
                    onValueChange={(value) => handleSelectChange("level", value)}
                  >
                    <SelectTrigger id="level">
                      <SelectValue placeholder={t("adminGrammar.selectLevel")} />
                    </SelectTrigger>
                    <SelectContent>
