...
const SpeakingQuestionForm: React.FC<SpeakingQuestionFormProps> = ({ 
  ...
  const hasAudioError = !!formState.error && !hasNewFile;
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-gray-700 font-semibold">
                {t("adminSpeakingQuestion.japaneseText")} <span className="text-rose-500">*</span>
              </FormLabel>
              <FormControl>
                <Textarea 
                  placeholder={t("adminSpeakingQuestion.japaneseTextPlaceholder")} 
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-gray-700 font-semibold">
                {t("adminSpeakingQuestion.vietnameseText")} <span className="text-rose-500">*</span>
              </FormLabel>
              <FormControl>
                <Textarea 
                  placeholder={t("adminSpeakingQuestion.vietnameseTextPlaceholder")} 
                  className="resize-y min-h-20"
                  {...field} 
                />
              </FormControl>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        <FormField
          control={form.control}
          name="sampleAnswerJapanese"
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-gray-700 font-semibold">
                {t("adminSpeakingQuestion.sampleAnswerJapanese")} <span className="text-rose-500">*</span>
              </FormLabel>
              <FormControl>
                <Textarea 
                  placeholder={t("adminSpeakingQuestion.sampleAnswerJapanesePlaceholder")} 
                  className="resize-y min-h-20"
                  {...field} 
                />
              </FormControl>
        ...
        <FormField
          control={form.control}
          name="sampleAnswerVietnamese"
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-gray-700 font-semibold">
                {t("adminSpeakingQuestion.sampleAnswerVietnamese")} <span className="text-rose-500">*</span>
              </FormLabel>
              <FormControl>
                <Textarea 
                  placeholder={t("adminSpeakingQuestion.sampleAnswerVietnamesePlaceholder")} 
                  className="resize-y min-h-20"
                  {...field} 
                />
              </FormControl>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-gray-700 font-semibold">
                {t("adminSpeakingQuestion.level")} <span className="text-rose-500">*</span>
              </FormLabel>
              <Select onValueChange={field.onChange} value={field.value}>
                <FormControl>
                  <SelectTrigger className="bg-white">
                    <SelectValue placeholder={t("adminSpeakingQuestion.levelPlaceholder")} />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
        ...
        <FormField
          ...
          render={({ field: { value, onChange, ...field } }) => (
            <FormItem>
              <FormLabel className="text-gray-700 font-semibold">
                {t("adminSpeakingQuestion.audio")} <span className="text-rose-500">*</span>
              </FormLabel>
              <div className="flex flex-col gap-2">
                <div className="flex flex-wrap gap-2">
                  ...
                  <div className="flex-1 overflow-hidden">
                    <Input 
                      value={fileName} 
                      readOnly 
                      placeholder={t("adminSpeakingQuestion.audioPlaceholder")}
                ...
                {hasAudioError && (
                  <p className="text-xs font-medium text-destructive">
                    {t("adminSpeakingQuestion.validationRequired", { field: t("adminSpeakingQuestion.audio") })}
                  </p>
                )}
                {hasExistingAudio && (
                  <p className="text-xs text-gray-500">
                    {t("adminSpeakingQuestion.audioDescription")}
                  </p>
                )}
              </div>
            </FormItem>
