...
const ListeningCard: React.FC<ListeningCardProps> = ({ content }) => {
    ...
    <div className="bg-white rounded-xl border border-gray-200 border-b-4 mb-5 overflow-hidden shadow-sm hover:shadow transition-shadow duration-300 cursor-pointer">
      ...
      >
        <div className="flex-1 flex flex-col p-5">
          <div className="text-sm text-gray-600 mb-2 font-bold text-primary">{t("learn_listening.script")}</div>
          <div className="flex justify-between items-start">
            <div className="flex-1">
              <div
                className="font-base text-gray-800 text-sm leading-8"
                dangerouslySetInnerHTML={{
                  __html:
                    content.japaneseTextFurigana || content.japaneseText || "",
                }}
              ></div>
              {content.sampleAnswerVietnamese && (
      ...
      {isUsageVisible && (
        <div className="w-full p-5 bg-gray-50 border-t border-gray-200">
          <div className="text-sm text-gray-700">{content.vietnameseText}</div>
        </div>
      )}
    </div>
  );
